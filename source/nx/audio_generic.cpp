...
void GenericAudio::BGM_Play(const std::string& file, int volume, int pitch, int fadein, bool loop) {
	bool bgm_set = false;
	for (unsigned i = 0; i < nr_of_bgm_channels; i++) {
		...
		if (!BGM_Channels[i].decoder && !bgm_set) {
			...
			BGM_PlayedOnceIndicator = false;
			UnlockMutex();
			PlayOnChannel(BGM_Channels[i], file, volume, pitch, fadein, loop);
		}
	}
}
...
int GenericAudio::SE_Play(std::string const &file, int volume, int pitch) {
	if (Muted) return -1;
	for (unsigned i = 0; i < nr_of_se_channels; i++) {
		if (!SE_Channels[i].se) {
...
void GenericAudio::SE_Stop(int chn) {
	SE_Channels[chn].stopped = true; //Stop all running sound effects
}
...
bool GenericAudio::PlayOnChannel(BgmChannel& chan, const std::string& file, int volume, int pitch, int fadein, bool loop) {
